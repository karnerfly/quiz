...
const DashboardHome = () => {
  ...
  const stats = [
    ...
    {
      title: "Active Users",
      value: calculateTotalSubmissions(),
      icon: faUsers,
      change: "+25%",
      color: "purple",
    },
    ...
    <main className="flex-1 ml-0 md:ml-6 transition-all duration-300 dark:text-white text-gray-800 overflow-x-hidden">
        ...
        <div className="relative z-10">
          ...
          {quizzes.length > 0 ? (
            <p className="text-gray-500 dark:text-gray-400 text-sm sm:text-base max-w-2xl">
              Manage your quizzes, polls and surveys. You have{" "}
              <span className="font-medium text-indigo-600">
                {quizzes[0]?.total_submissions | 0} submissions{" "}
              </span>
              in latest quiz
            </p>
          ) : (
      ...
      <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-5 mb-6">
        {features.map((feature, index) => (
          ...
          >
            <div
              className={`w-10 h-10 sm:w-12 sm:h-12 rounded-lg ${feature.color} flex items-center justify-center mb-3 sm:mb-4 text-lg sm:text-xl`}
            >
          ...
          >
            <div className="flex justify-between items-start">
              <div>
                ...
                <h3
                  className={`text-2xl sm:text-3xl font-bold mt-1 bg-gradient-to-r from-${stat.color}-600 to-${stat.color}-400 bg-clip-text text-${stat.color}-600`}
                >
                  {stat.value}
                </h3>
              </div>
              <div
                className={`w-8 h-8 sm:w-10 sm:h-10 rounded-lg bg-${stat.color}-100 flex items-center justify-center`}
              >
                <FontAwesomeIcon
                  icon={stat.icon}
            ...
            {/* <div
              className={`mt-3 sm:mt-4 flex items-center text-xs font-medium text-${
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
        ...
        <div className="lg:col-span-2 rounded-xl shadow-lg p-4 sm:p-6 dark:bg-gray-800 border dark:border-gray-700 bg-white">
          <div className="flex justify-between items-center mb-4 sm:mb-5">
            ...
            <Link
              to="activity"
              className="text-xs sm:text-sm text-indigo-600 hover:text-indigo-800 dark:hover:text-indigo-400"
            >
              View All
            </Link>
          </div>
          <div className="space-y-3 sm:space-y-4">
            {recentActivity.map((activity, index) => (
              ...
              >
                <div
                  className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full ${activity.iconColor} flex items-center justify-center mt-1 mr-3`}
                >
                  <FontAwesomeIcon
                    icon={activity.icon}
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-medium dark:text-gray-200 text-sm sm:text-base truncate">
                    {activity.type}
                  </p>
